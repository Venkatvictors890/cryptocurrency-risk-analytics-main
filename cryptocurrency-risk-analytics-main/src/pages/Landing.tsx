...
const steps = [
  ...
  { num: "04", title: "Act", desc: "Get actionable recommendations — Strong Buy, Hold, or Avoid." },
];
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <nav className="flex items-center justify-between px-6 py-4 border-b border-border/40 bg-background/80 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-3">
          <ThemeToggle />
          <Link to="/markets">
            <Button variant="ghost" size="sm" className="text-muted-foreground hover:text-foreground">Markets</Button>
          </Link>
          <Link to="/dashboard">
      ...
      <section className="relative flex-1 flex flex-col items-center justify-center text-center px-6 py-24 overflow-hidden">
        ...
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/8 border border-primary/15 text-xs text-primary mb-6 animate-fade-up">
          ...
          Smart Crypto Analysis Platform
        </div>
        ...
        <p className="mt-5 text-muted-foreground text-base sm:text-lg max-w-xl leading-relaxed animate-fade-up" style={{ animationDelay: "0.2s" }}>
          Analyze risk, sentiment, and market behavior in real-time. Make smarter investment decisions with AI-powered insights.
        </p>

        <div className="flex flex-wrap gap-3 mt-8 animate-fade-up" style={{ animationDelay: "0.3s" }}>
          <Link to="/dashboard">
            <Button size="lg" className="gap-2 text-sm h-11 px-6">
              Enter Dashboard <ChevronRight className="h-4 w-4" />
            </Button>
          </Link>
          <Link to="/markets">
            <Button size="lg" variant="outline" className="text-sm h-11 px-6">
              View Markets
            </Button>
          </Link>
        </div>
      ...
      <section className="px-6 py-20 border-t border-border/40">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {features.map((f) => (
              <div key={f.title} className="glass-card rounded-xl p-5 group hover:border-primary/20 transition-all">
...
function PreviewCards() {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {cryptos.map((c) => (
        <div key={c.id} className="glass-card rounded-xl p-4 text-left">
